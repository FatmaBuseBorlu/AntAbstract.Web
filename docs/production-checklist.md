...
- [ ] DB şifresi rotasyonu yapıldı (geçmişte depoya girmiş eski şifre **kullanılmamalı**)
- [ ] Bekleyen migration'lar kontrol edildi (uygulama açılışta bekleyen migration'ları kendisi uygular; büyük migration'lar öncesi yedek alın)
...
- [ ] SMTP kimlik bilgileri rotasyonu yapıldı (geçmişte depoya girmiş eski kullanıcı **kullanılmamalı**)
...
- [ ] `SENTRY_DSN` girildi; Sentry'de test hatası görüldü (boşsa hata izleme kapalıdır)
...
- [ ] Tüm testler yeşil (`dotnet test` — Infrastructure + Web, 0 hata)
...
- [ ] `/health` (anahtarsız, 200/503) uptime aracına (ör. UptimeRobot) tanımlandı
