...
namespace AntAbstract.Infrastructure.Services
{
    public class CertificateService : ICertificateService
    {
        ...
        {
            var q = _context.Certificates.AsNoTracking().Where(x => x.UserId == userId);
        ...
        public async Task<byte[]?> GetCertificateFileAsync(Guid certificateId, string userId)
        {
            ...
            var abs = Path.Combine(_env.WebRootPath, cert.FilePath.TrimStart('/').Replace("/", Path.DirectorySeparatorChar.ToString()));
        ...
        private async Task EnsureCertificateAsync(Guid conferenceId, string userId, CertificateType type)
        {
            ...
            if (type == CertificateType.Author)
            {
                ...
                var isAuthorInConference =
                    await _context.Submissions.AsNoTracking().AnyAsync(s => s.ConferenceId == conferenceId && s.AuthorId == userId)
                    || await _context.SubmissionAuthors.AsNoTracking().AnyAsync(a => a.Submission.ConferenceId == conferenceId && a.AppUserId == userId);
            ...
            if (existing.GeneratedAt == null || string.IsNullOrWhiteSpace(existing.FilePath))
            {
                ...
                var pdfBytes = GeneratePdf(conference, user, type);

                var tenantSlug = conference.Tenant?.Slug ?? "tenant";
                var safeTenant = Slugify(tenantSlug);
                var safeConf = Slugify(conference.Title ?? "conference");
        ...
        private static byte[] GeneratePdf(Conference conf, AppUser user, CertificateType type)
        {
            QuestPDF.Settings.License = LicenseType.Community;

            var title = type == CertificateType.Author ? "Yazar Katılım Sertifikası" : "Hakem Sertifikası";
            var fullName = $"{user.FirstName} {user.LastName}".Trim();
            if (string.IsNullOrWhiteSpace(fullName)) fullName = user.UserName ?? user.Email ?? "Kullanıcı";
            ...
            var bytes = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    {
                        col.Item().Text(conf.Title ?? "Konferans").FontSize(20).Bold();
                        col.Item().Text(title).FontSize(18).SemiBold();
                        col.Item().PaddingTop(20).Text($"{fullName}").FontSize(16).Bold();
                        col.Item().PaddingTop(10).Text("Bu sertifika ilgili süreç koşulları sağlandığı için oluşturulmuştur.");
